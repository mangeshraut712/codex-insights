...
Trust & Coverage records the selected data source, discovered and eligible populations, analyzed count, source and short-thread exclusions, read failures, sampling count, fallback warnings, analysis mode, and redaction count. Missing or excluded sessions can make deterministic metrics and model narratives incomplete.

Main interactive sessions are included by default. Delegated and subagent sources are excluded unless `--include-subagents` is explicit. This prevents delegated work from being attributed to the primary user by default.

## Storage and deletion

Reports default to `~/.codex/usage-data/report.html` and `report.json`. Facet caches default to `~/.codex-insights-cache`; session-summary caches are stored beneath the selected cache root. A custom output or cache directory may be selected with `--out-dir` and `--cache-dir`.
