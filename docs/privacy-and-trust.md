...
The optional `profile` command writes a self-contained HTML snapshot and a `profile.json` locally. By default it reads the same account-wide aggregates the Codex app shows under Settings → Profile (through the Codex app-server and your existing ChatGPT sign-in); with `--source local`, or when account stats are unavailable, it uses only aggregate totals, model counts, and session dates from a local report. It never reads transcripts for the profile and never uploads anything. A profile becomes public only after you put the exported `index.html` on a web host. See [Shareable profile](shareable-profile.md).
