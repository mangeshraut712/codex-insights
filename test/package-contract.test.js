...
test('install and owner-auth scripts stay token-free and documented', async () => {
  ...
  assert.match(install, /resolve_npm_prefix/)
  assert.match(install, /resolve_codex_bin/)
  assert.match(install, /\/opt\/homebrew\/bin\/codex/)
  assert.match(install, /Darwin/)
  ...
  assert.match(installDocs, /macOS/)
  assert.match(installDocs, /\/opt\/homebrew\/bin/)
