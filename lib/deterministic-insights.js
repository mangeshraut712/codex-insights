...
export function buildDeterministicInsights(report) {
  ...
  const overlapEvents = Number(report.summary?.overlap?.overlapEvents || 0)
  const overlapSummary = overlapEvents
    ? `${formatCount(overlapEvents, 'cross-session 30-minute overlap')} appeared in user-message timestamps; this does not establish switching cost.`
    : 'No cross-session 30-minute user-message overlaps were detected.'
  ...
  return {
    ...
    at_a_glance: {
      basis: 'deterministic',
      whats_working: `${formatCount(threadCount, 'substantive session')} ${threadCount === 1 ? 'was' : 'were'} analyzed. ${capabilitySummary}`,
    ...
    interaction_style: {
      basis: 'deterministic',
      narrative: `${concentrationSummary} ${overlapSummary} ${analysisLimit}`,
