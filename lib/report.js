...
export function renderTerminalSummary(report) {
  ...
  lines.push(
    `  mode=${report.analysisMode || 'unknown'} | source=${coverage?.dataSource || 'unknown'} | discovered=${coverage?.discovered ?? report.metadata.threadCount} | eligible=${coverage?.eligible ?? report.metadata.threadCount} | analyzed=${coverage?.analyzed ?? report.metadata.threadCount} | failed=${coverage?.failedToRead ?? 0} | redactions=${report.privacy?.redactions ?? 0}`,
  )
