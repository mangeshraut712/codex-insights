...
export const REPORT_COPY_FILENAME = /^report-(\d{4}-\d{2}-\d{2}T\d{6}Z)\.(html|json)$/

export function formatReportCopyStamp(generatedAt) {
...
export function parseReportCopyStamp(stamp) {
...
export function formatSessionCoveragePhrase(report, text) {
  const coverage = report.metadata?.coverage
  const analyzed = coverage?.analyzed ?? report.metadata.threadCount
  const discovered = coverage?.discovered
  const unit = analyzed === 1 ? text.sessionSingular : text.sessionPlural
  const analyzedLabel = `${analyzed} ${unit}`
  if (typeof discovered === 'number' && discovered > analyzed) {
    return text.sessionCoverageWithTotal
      .replaceAll('{analyzedLabel}', analyzedLabel)
      .replaceAll('{total}', String(discovered))
  }
  return analyzedLabel
}

export async function cleanupExpiredReportCopies(outDir, options = {}) {
  const retentionDays = options.retentionDays ?? DEFAULT_REPORT_RETENTION_DAYS
  ...
  for (const name of names) {
    const match = name.match(REPORT_COPY_FILENAME)
    if (!match) continue
    if (options.keepStamp && match[1] === options.keepStamp) continue
    const stamped = parseReportCopyStamp(match[1])
    if (!stamped) continue
    if (now.getTime() - stamped.getTime() > maxAgeMs) {
...
export function buildReport(threadSummaries, options = {}) {
  ...
  for (const thread of threadSummaries) {
    const activityDay = String(thread.updatedAt || '').slice(0, 10)
    if (/^\d{4}-\d{2}-\d{2}$/.test(activityDay)) increment(activityDayCounts, activityDay)
  ...
  return {
    schemaVersion: 2,
    analysisMode: options.analysisMode ?? 'model-assisted',
    privacy: {
      redactions: threadSummaries.reduce(
        (sum, thread) => sum + Number(thread.redactions || 0),
        0,
      ),
    },
    metadata: {
    ...
    charts: {
      ...
      activityDays: Object.entries(activityDayCounts).sort(([a], [b]) => a.localeCompare(b)).map(([date, sessions]) => ({ date, sessions })),
    },
...
export async function writeReportFiles(report, options) {
  ...
  const stamp = formatReportCopyStamp(sanitizedReport.metadata?.generatedAt)
  let archiveHtmlPath
  let archiveJsonPath
  if (stamp && options.archiveCopies !== false) {
    archiveHtmlPath = path.join(outDir, `report-${stamp}.html`)
    archiveJsonPath = path.join(outDir, `report-${stamp}.json`)
    await fs.copyFile(htmlPath, archiveHtmlPath)
    await fs.copyFile(jsonPath, archiveJsonPath)
    await cleanupExpiredReportCopies(outDir, {
      now: options.now,
      retentionDays: options.retentionDays ?? DEFAULT_REPORT_RETENTION_DAYS,
      keepStamp: stamp,
    })
  }

  return { jsonPath, htmlPath, archiveHtmlPath, archiveJsonPath, sanitizedReport }
}
...
export function renderTerminalSummary(report) {
  ...
  lines.push(formatSessionCoveragePhrase(report, text))
  lines.push(
  ...
  const coverage = report.metadata.coverage
  lines.push(text.trustAndCoverage)
  lines.push(
    `  mode=${report.analysisMode || 'unknown'} | source=${coverage?.dataSource || 'unknown'} | discovered=${coverage?.discovered ?? report.metadata.threadCount} | eligible=${coverage?.eligible ?? report.metadata.threadCount} | analyzed=${coverage?.analyzed ?? report.metadata.threadCount} | reused=${coverage?.reused ?? 0} | unseen=${coverage?.unseen ?? 0} | overCap=${coverage?.excludedUnseenOverCap ?? 0} | failed=${coverage?.failedToRead ?? 0} | redactions=${report.privacy?.redactions ?? 0}`,
  )
  ...
  <body>
    <main>
      <section class="hero">
        ...
        <p class="meta">${escapeHtml(renderHeroMeta(report, text))}</p>
        ...
        <nav class="report-nav">
          <a href="#section-trust">${escapeHtml(text.trustAndCoverage)}</a>
...
function renderHeroMeta(report, text) {
  const coverage = report.metadata.coverage
  const skipped = coverage?.excludedShort > 0 ? ` ${text.skippedShortSessions}` : ''
  return `${text.generatedLabel} ${report.metadata.generatedAt}. ${formatSessionCoveragePhrase(report, text)} ${text.inCodexHome} ${formatCodexHome(report.metadata.codexHome)}.${skipped}`
}
...
function renderTrustAndCoverage(report) {
  const text = getReportText(report.metadata.language)
  const coverage = report.metadata.coverage
  const mode =
    report.analysisMode === 'local-only' ? text.localOnlyMode : text.modelAssistedMode
  const basis =
    report.insights?.basis === 'deterministic' ? text.deterministicBasis : text.modelBasis
  const warnings = coverage?.warnings || []

  return `
    <section class="panel" id="section-trust">
      <h2>${escapeHtml(text.trustAndCoverage)}</h2>
      <p class="section-intro">${escapeHtml(text.trustAndCoverageIntro)}</p>
      <div class="summary-grid">
        ${renderStat(text.analysisMode, mode)}
        ${renderStat(text.findingBasis, basis)}
        ${renderStat(text.dataSource, coverage?.dataSource || text.unknown)}
        ${renderStat(text.discovered, formatNumber(coverage?.discovered || 0))}
        ${renderStat(text.eligible, formatNumber(coverage?.eligible || 0))}
        ${renderStat(text.analyzed, formatNumber(coverage?.analyzed || report.metadata.threadCount))}
        ${renderStat(text.reused, formatNumber(coverage?.reused || 0))}
        ${renderStat(text.unseen, formatNumber(coverage?.unseen || 0))}
        ${renderStat(text.unseenAnalyzed, formatNumber(coverage?.unseenAnalyzed || 0))}
        ${renderStat(text.excludedUnseenOverCap, formatNumber(coverage?.excludedUnseenOverCap || 0))}
        ${renderStat(text.excludedShort, formatNumber(coverage?.excludedShort || 0))}
        ${renderStat(text.excludedSource, formatNumber(coverage?.excludedSource || 0))}
        ${renderStat(text.failedToRead, formatNumber(coverage?.failedToRead || 0))}
        ${renderStat(text.redactions, formatNumber(report.privacy?.redactions || 0))}
      </div>
      ${
        warnings.length
          ? `<div class="key-insight"><strong>${escapeHtml(text.warnings)}:</strong><ul>${warnings
...
function getReportText(lang) {
  if (lang === 'zh-CN') {
    return {
      ...
      sessionSingular: '个会话',
      sessionPlural: '个会话',
      sessionCoverageWithTotal: '{analyzedLabel}（共 {total} 个）',
      skippedShortSessions: '已跳过过短会话。',
      ...
      timeOfDay: '活跃时段',
      trustAndCoverage: '信任与覆盖范围',
      trustAndCoverageIntro: '这些字段说明报告读取了什么、排除了什么，以及结论如何生成。',
      analysisMode: '分析模式',
      findingBasis: '结论依据',
      dataSource: '数据来源',
      discovered: '发现线程',
      ...
      warnings: '警告',
      localOnlyMode: '仅本地',
      modelAssistedMode: '模型辅助',
      deterministicBasis: '确定性',
      modelBasis: '模型解释',
      unknown: '未知',
      atAGlance: '一眼看懂',
      whatsWorking: '目前顺的地方',
  ...
  return {
    ...
    sessionCoverageWithTotal: '{analyzedLabel} ({total} total)',
    skippedShortSessions: 'Very short sessions were skipped.',
    inCodexHome: 'in',
    ...
    trustAndCoverage: 'Trust & Coverage',
    trustAndCoverageIntro:
      'These fields show what the report read, what it excluded, and how its findings were produced.',
    analysisMode: 'Analysis Mode',
    findingBasis: 'Finding Basis',
    ...
    reused: 'Reused',
    unseen: 'Unseen',
    unseenAnalyzed: 'Newly Analyzed',
    excludedUnseenOverCap: 'Unseen Over Cap',
    excludedShort: 'Excluded Short',
    excludedSource: 'Excluded Source',
    failedToRead: 'Failed to Read',
    redactions: 'Redactions',
    warnings: 'Warnings',
    localOnlyMode: 'Local-only',
    modelAssistedMode: 'Model-assisted',
    deterministicBasis: 'Deterministic',
    modelBasis: 'Model interpretation',
    unknown: 'Unknown',
