...
const hoursLabel = value => new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(Math.max(0, Number(value) || 0))
...
export function buildPublicProfile(report, options = {}) {
  ...
  return {
    ...
    since: /^\d{4}-\d{2}-\d{2}$/.test(report.metadata.since) ? report.metadata.since : '',
    ...
    tokenMix: {
      input: number(report.summary.totalInputTokens),
      cachedInput: number(report.summary.totalCachedInputTokens),
      output: number(report.summary.totalOutputTokens),
      reasoningOutput: number(report.summary.totalReasoningOutputTokens),
    },
...
export function renderPublicProfile(profile) {
  ...
  const scope = profile.since ? `Since ${profile.since}` : profile.scopeDays ? `Last ${profile.scopeDays} days` : 'All available local history'
  const tokenMixRows = renderTokenMix(profile.tokenMix)
...
<section class="stats" aria-label="Profile statistics"><div class="stat"><strong>${compact(profile.tokens)}</strong><span>Tokens in analyzed sessions</span></div><div class="stat"><strong>${compact(profile.sessions)}</strong><span>Analyzed sessions</span></div><div class="stat"><strong>${compact(profile.activityDays.length)}</strong><span>Active days in report</span></div><div class="stat"><strong>${hoursLabel(profile.hours)}h</strong><span>Session duration total</span></div></section>
...
${tokenMixRows ? `<div class="section-head"><h2>Token mix</h2><span>Across analyzed sessions</span></div><section class="panel"><ol class="models">${tokenMixRows}</ol></section>` : ''}
...
<footer class="foot">${escapeHtml(scope)}${range ? ` · ${escapeHtml(range)}` : ''} · ${escapeHtml(coverage)}<br>Generated ${escapeHtml(profile.generatedAt || 'from a local report')}. This page contains aggregate data only and does not update automatically. Built with <a href="https://github.com/mangeshraut712/codex-insights">Codex Insights</a>.</footer></main><script>const shareButton=document.getElementById('share-link');shareButton.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(location.href);shareButton.textContent='Copied'}catch{shareButton.textContent='Copy URL from address bar'}})</script></body></html>`
}

function renderTokenMix(mix) {
  if (!mix || !mix.input) return ''
  const share = (part, whole) => whole ? `${Math.round((part / whole) * 100)}%` : '0%'
  const rows = [
    ['Input tokens', compact(mix.input)],
    ['Served from cache', share(mix.cachedInput, mix.input)],
    ['Output tokens', compact(mix.output)],
    ['Reasoning share of output', share(mix.reasoningOutput, mix.output)],
  ]
  return rows.map(([label, value], index) => `<li><span class="rank">0${index + 1}</span><span>${label}</span><strong>${value}</strong></li>`).join('')
}
