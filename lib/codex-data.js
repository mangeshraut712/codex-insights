...
import { extractTokenSnapshot } from './rollout-tokens.js'
...
async function collectLegacyThreadData(options) {
  ...
  return loadUnseenAwarePopulation({
    eligibleThreads,
    coverage,
    unseenLimit: Number(options.limit ?? 0),
    ...
    readSummary: async thread => {
      ...
      if (hasRolloutReadFailure(summary)) {
        throw new Error('rollout_read')
      }
      return summary
    },
  })
