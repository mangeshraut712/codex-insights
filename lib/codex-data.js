...
import { redactSensitiveValue, redactSessionSummary } from './redaction.js'
...
async function collectLegacyThreadData(options) {
  ...
  while (true) {
    ...
    for (const thread of threads) {
      if (!options.includeSubagents && isLegacyDelegatedThread(thread)) {
        excludedSource += 1
        continue
      }
  ...
  return {
    summaries: selected,
    coverage: legacyCoverage({ discovered, summaries, selected, excludedSource }),
  }
}

function legacyCoverage({ discovered, summaries, selected, excludedSource = 0 }) {
  const failedToRead = summaries.filter(hasRolloutReadFailure).length
  return {
    dataSource: 'legacy',
    discovered,
    eligible: summaries.length,
    analyzed: selected.length,
    excludedShort: summaries.filter(
      summary => !hasRolloutReadFailure(summary) && !isSubstantiveThread(summary),
    ).length,
    excludedSource,
    failedToRead,
...
function hasRolloutReadFailure(summary) {
  return Number(summary?.toolErrorCategories?.rollout_read ?? 0) > 0
}

function isLegacyDelegatedThread(thread) {
  if (thread?.agentRole || thread?.agentNickname || thread?.agentPath) return true
  const source = String(thread?.source ?? '').trim()
  if (!source) return false
  if (/sub[_-]?agent/i.test(source)) return true
  try {
    const parsed = JSON.parse(source)
    return Boolean(parsed?.subagent || parsed?.subAgent)
  } catch {
    return false
  }
}
...
async function loadCachedOrFreshSummary(thread, cacheDir, options) {
  ...
  const versionKey = buildSessionSummaryVersionKey(thread, options)
...
function buildSessionSummaryVersionKey(thread, options) {
...
export const __test = {
  buildSessionSummaryVersionKey,
  isLegacyDelegatedThread,
  legacyCoverage,
}
