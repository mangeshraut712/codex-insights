...
export async function collectThreadData(options = {}) {
  ...
  if (dataSource === 'legacy') {
    return collectLegacyThreadData(options)
  }
  if (dataSource !== 'auto') {
  ...
  try {
    ...
    const legacy = await collectLegacyThreadData(options)
...
async function collectLegacyThreadData(options) {
  ...
  if (substantiveTarget > 0) {
    const filtered = filterSubstantiveThreads(summaries).slice(0, substantiveTarget)
    return {
      summaries: filtered,
      coverage: legacyCoverage(filtered),
    }
  }
  return {
    summaries,
    coverage: legacyCoverage(summaries),
  }
}

function legacyCoverage(summaries) {
  return {
    dataSource: 'legacy',
