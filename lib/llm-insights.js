...
export async function generateLlmInsights({ threadSummaries, options = {} }) {
  ...
  const candidateThreads = filterSubstantiveThreads(
    threadSummaries.map(thread => redactSensitiveValue(thread).value),
  )
  ...
  for (const section of sectionResults) {
    insights[section.name] = redactSensitiveValue(section.result).value
  }
  ...
  insights.at_a_glance = redactSensitiveValue(atAGlance).value

  return {
    insights: redactSensitiveValue(insights).value,
    facets,
    reportThreads,
    facetSelection: {
...
async function readCachedFacet(thread, { cacheDir, model }) {
  ...
  const redacted = redactSensitiveValue(cached.facet)
...
async function getFacetForThread(thread, { cacheDir, model, provider, providerOptions }) {
  ...
  if (cached?.versionKey === versionKey && cached?.facet) {
    const facet = redactSensitiveValue(cached.facet).value
    await fs.writeFile(cachePath, JSON.stringify({ ...cached, facet }, null, 2), 'utf8')
    return facet
  }
...
async function prepareTranscriptForFacetExtraction(thread, { model, provider, providerOptions }) {
  ...
  const chunkSummaries = await mapLimit(chunks, 3, async (chunk, index) => {
    ...
    return redactSensitiveText(summary).text
  })
  ...
  return `[Long transcript summarized before facet extraction]\n${redactSensitiveText(finalSummary).text.trim()}`
}
