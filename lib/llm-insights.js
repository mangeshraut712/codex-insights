...
import { selectRepresentativeThreads } from './sampling.js'
...
export async function generateLlmInsights({ threadSummaries, options = {} }) {
  ...
  const candidateThreads = filterSubstantiveThreads(
    threadSummaries.map(thread => redactSensitiveValue(thread, { homeDir: options.homeDir }).value),
  )
  ...
  let completedFacetCount = 0
  const completedFacetJobs = await mapLimit(facetJobs, 4, async job => {
    const facet =
      job.cachedFacet ||
    ...
    completedFacetCount += 1
    emitProgress(options, {
      kind: 'facets:progress',
      completed: completedFacetCount,
      total: facetJobs.length,
      cached: Boolean(job.cachedFacet),
    })
    return { facet, cached: Boolean(job.cachedFacet) }
  })

  const minimalThreadIds = new Set(
    completedFacetJobs.filter(job => isWarmupMinimalFacet(job.facet)).map(job => job.facet.threadId),
  )
  const reportThreads = candidateThreads.filter(thread => !minimalThreadIds.has(thread.id))
  const selectedFacetJobs = completedFacetJobs.filter(job => !minimalThreadIds.has(job.facet.threadId))
  const facets = selectedFacetJobs.map(job => job.facet)
  ...
  for (const section of sectionResults) {
    insights[section.name] = redactSensitiveValue(section.result, { homeDir: options.homeDir }).value
  }
  ...
  insights.at_a_glance = redactSensitiveValue(atAGlance, { homeDir: options.homeDir }).value

  return {
    insights: redactSensitiveValue(insights, { homeDir: options.homeDir }).value,
    facets,
    reportThreads,
    facetSelection: {
      cached: selectedFacetJobs.filter(job => job.cached).length,
      new: selectedFacetJobs.filter(job => !job.cached).length,
    },
    analysisUsage: usageTracker.snapshot(),
  }
}
...
async function planFacetJobs(threadSummaries, { cacheDir, model, uncachedLimit, homeDir }) {
  const cachedJobs = []
  const uncachedThreads = []

  for (const thread of threadSummaries) {
    const cachedFacet = await readCachedFacet(thread, { cacheDir, model, homeDir })
    if (cachedFacet) {
      cachedJobs.push({ thread, cachedFacet })
      continue
    }
    uncachedThreads.push(thread)
  }

  return [
    ...cachedJobs,
    ...selectRepresentativeThreads(uncachedThreads, uncachedLimit).map(thread => ({
      thread,
      cachedFacet: null,
    })),
  ]
...
async function readCachedFacet(thread, { cacheDir, model, homeDir }) {
  const cachePath = path.join(cacheDir, `${thread.id}.json`)
  const cached = await readJson(cachePath)
  if (!cached?.facet) return null
  const redacted = redactSensitiveValue(cached.facet, { homeDir })
  const facet = redacted.value
  if (redacted.redactions > 0) {
    await fs.writeFile(cachePath, JSON.stringify({ ...cached, facet }, null, 2), 'utf8')
  }
  if (cached.versionKey !== buildFacetVersionKey(thread, model)) return null
  return facet
}
...
async function getFacetForThread(thread, { cacheDir, model, provider, providerOptions }) {
  ...
  if (cached?.versionKey === versionKey && cached?.facet) {
    const facet = redactSensitiveValue(cached.facet, { homeDir: providerOptions.homeDir }).value
    await fs.writeFile(cachePath, JSON.stringify({ ...cached, facet }, null, 2), 'utf8')
    return facet
  }
  ...
  const facet = redactSensitiveValue({
    threadId: thread.id,
    title: thread.title,
    cwd: thread.cwd,
    ...
    ...rawFacet,
  }, { homeDir: providerOptions.homeDir }).value
...
async function prepareTranscriptForFacetExtraction(thread, { model, provider, providerOptions }) {
  ...
  const chunkSummaries = await mapLimit(chunks, 3, async (chunk, index) => {
    const summary = await callTextModel({
      provider,
      model,
      systemPrompt:
    ...
    return redactSensitiveText(summary, { homeDir: providerOptions.homeDir }).text
  })
  ...
  return `[Long transcript summarized before facet extraction]\n${redactSensitiveText(finalSummary, { homeDir: providerOptions.homeDir }).text.trim()}`
}
