...
export async function collectAppServerThreadSummaries(options = {}) {
  ...
  try {
    ...
    for (const thread of eligibleThreads) {
      if (limit && substantiveCount >= limit) break
      sampledCount += 1
      try {
        ...
        const summary = summarizeAppServerThread(response.thread ?? thread)
        collectedSummaries.push(summary)
        if (isSubstantiveThread(summary)) substantiveCount += 1
        else coverage.excludedShort += 1
      } catch (error) {
        if (isAppServerProtocolError(error)) throw error
        coverage.failedToRead += 1
      }
    }
    const substantiveSummaries = filterSubstantiveThreads(collectedSummaries)
    const summaries = limit ? substantiveSummaries.slice(0, limit) : substantiveSummaries
    coverage.analyzed = summaries.length
    coverage.sampled = sampledCount
...
export function summarizeAppServerThread(thread) {
  const toolCounts = {}
