...
import { emptyTokenUsage, readRolloutTokenUsage } from './rollout-tokens.js'
  ...
  try {
    ...
    return await loadUnseenAwarePopulation({
      ...
      readSummary: async thread => {
        ...
        const fullThread = response.thread ?? thread
        const tokenUsage = await readRolloutTokenUsage(fullThread.path ?? thread.path)
        return summarizeAppServerThread(fullThread, options, tokenUsage)
      },
    })
  } finally {
    client.close()
...
export function summarizeAppServerThread(thread, options = {}, tokenUsage = emptyTokenUsage()) {
  const toolCounts = {}
